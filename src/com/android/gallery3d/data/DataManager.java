...
public class DataManager {
    ...
    public DataManager(OpenApp application) {
    ...
    public MediaObject peekMediaObject(OpenPath path) {
    ...
    public MediaObject getMediaObject(OpenPath path) {
        MediaObject obj = path.getObject();
        if (obj != null) return obj;

        MediaSource source = mSourceMap.get(path.getParent().getPath());
        if (source == null) {
    ...
    public MediaObject getMediaObject(String s) {
        return getMediaObject(OpenPath.fromString(s));
    }

    public MediaSet getMediaSet(OpenPath path) {
    ...
    // startIndex).
    public void mapMediaItems(ArrayList<OpenPath> list, ItemConsumer consumer,
            int startIndex) {
    ...
    public int getSupportedOperations(OpenPath path) {
        return getMediaObject(path).getSupportedOperations();
    }

    public void delete(OpenPath path) {
        getMediaObject(path).delete();
    }

    public void rotate(OpenPath path, int degrees) {
        getMediaObject(path).rotate(degrees);
    }

    public Uri getContentUri(OpenPath path) {
        return getMediaObject(path).getContentUri();
    }

    public int getMediaType(OpenPath path) {
        return getMediaObject(path).getMediaType();
    }

    public MediaDetails getDetails(OpenPath path) {
        return getMediaObject(path).getDetails();
    }

    public void cache(OpenPath path, int flag) {
        getMediaObject(path).cache(flag);
    }

    public OpenPath findPathByUri(Uri uri) {
        if (uri == null) return null;
        for (MediaSource source : mSourceMap.values()) {
            OpenPath path = source.findPathByUri(uri);
