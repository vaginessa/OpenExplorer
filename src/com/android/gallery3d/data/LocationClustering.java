...
class LocationClustering extends Clustering {
    ...
    public void run(MediaSet baseSet) {
        ...
        if (hasUnresolvedAddress) {
            mHandler.post(new Runnable() {
                public void run() {
                    Toast.makeText(mContext, R.string.s_error_ftp,
    ...
    @Override
    public ArrayList<OpenPath> getCluster(int index) {
        ArrayList<SmallItem> items = mClusters.get(index);
        ArrayList<OpenPath> result = new ArrayList<OpenPath>(items.size());
