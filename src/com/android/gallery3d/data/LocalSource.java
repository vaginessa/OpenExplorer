...
class LocalSource extends MediaSource {
    ...
    public LocalSource(OpenApp context) {
        super("local");
    ...
    @Override
    public MediaObject createMediaObject(OpenPath path) {
        OpenApp app = mApplication;
    ...
    private OpenPath getAlbumPath(Uri uri, int defaultType) {
        ...
        switch (mediaType) {
            case MEDIA_TYPE_IMAGE:
                return OpenPath.fromString("/local/image").getChild(id);
            case MEDIA_TYPE_VIDEO:
                return OpenPath.fromString("/local/video").getChild(id);
            default:
                return OpenPath.fromString("/local/all").getChild(id);
        }
    }
    ...
    public void mapMediaItems(ArrayList<PathId> list, ItemConsumer consumer) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            OpenPath parent = pid.path.getParent();
            if (parent.equals(LocalImage.ITEM_PATH)) {
                imageList.add(pid);
            } else if (parent.equals(LocalVideo.ITEM_PATH)) {
                videoList.add(pid);
            }
        }
